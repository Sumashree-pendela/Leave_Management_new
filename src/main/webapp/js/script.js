...
teamLeavesTab.addEventListener('click', function() {
    ...
    // Fetch and display team leave requests
    saveTeamLeaves();
});
...
const leaveNameSelect = document.getElementById('leaveName');
 ...
 const dateFromInput = document.getElementById('dateFrom');
     ...
     //Updating 'Date To' based on 'Date From'
      ...
      console.log("Inside fetch profile..");
            fetch('http://localhost:8080/leave_management//leave/employee?profile=1')
                ...
                console.log(data.gender);
                    // Assuming the JSON response has a field called 'name'
...
// Define the createLeaveRequest method
...
const newStatus = leaveStatusMapping[status] || status;
    // Create a new leave item row
    const leaveItem = document.createElement('div');
    leaveItem.className = 'row mb-3 leave-item'; // Add a class for identification
    ...
    // Append the new leave item to the container
...
const newStatus = leaveStatusMapping[status] || status;
    ...
    // Create and append each div element
...
if (status === 'PENDING') {
        ...
        // Add class to hide button container if status is not 'PENDING'
        teamLeaveItem.classList.add('no-buttons');
    }

   teamLeaveItem.appendChild(buttonContainer);

    // Finally, append the teamLeaveItem to the teamLeaveContainer
    teamLeaveContainer.appendChild(teamLeaveItem);

    detailsButtonEventListener(detailsButton, appliedBy);

    // Call the event listener function only if buttons are created
    ...
    .then(data => {
        ...
        data.forEach(teamLeaveTracker => {
         ...
         const buttonContainer = document.createElement('div');
                ...
                // Append the button container to the dialog
